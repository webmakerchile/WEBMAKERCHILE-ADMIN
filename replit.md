...
- **Cover Generator**: AI-powered cover image generation using OpenAI gpt-image-1 with reference images. Sistema de diseño v2 (julio 2026):
  - **Plantillas de composición** (`src/lib/thumbnail-templates.ts`): formatos predeterminados que se intercalan con rotación FIFO — YouTube 16:9 (clásica lateral, lateral espejo, cara gigante, impacto inferior, testimonio con comillas, top con número gigante) y vertical 9:16 (titular superior/inferior). Cada plantilla define el bloque de composición del prompt (zona despejada), la zona de texto del overlay, el lado del scrim y extras decorativos.
  - **Motor de tipografía de impacto** (`src/lib/title-style.ts`): 7 estilos de titular estilo youtuber (Impacto/MrBeast, Titán con placa, 3D cartel, Neón, Degradado dorado, Placas, Clásico) renderizados como capas de `<text>` librsvg-safe (contorno, sombra dura, extrusión, glow, gradiente). Layout con MÉTRICAS REALES por carácter (`font-metrics.generated.ts`, calibradas con `scripts/calibrate-font-metrics.ts` rasterizando con librsvg): cada línea se parte balanceada (partición lineal DP) y ajusta su font-size para llenar el ancho exacto de la columna. Fuentes nuevas empaquetadas: Anton, Archivo Black, Bebas Neue, Alfa Slab One, Passion One Black (OFL, en `assets/fonts`).
  - **Fidelidad de rostro multi-foto**: `/api/gemini/generate-youtube-cover` acepta `personImagesBase64` (hasta 3 ángulos de la misma persona); el adaptador gemini-ai pasa TODAS las referencias a `images.edit` (antes descartaba todas menos la primera) con `input_fidelity: "high"`; el bloque de identidad del prompt exige usar todos los ángulos.
  - **API**: `cover-options` expone `plantillas` y `estilosTitular`; `generate-cover`/`generate-youtube-cover` aceptan `plantillaId`/`estiloTitularId`; `improve-cover-idea` sugiere plantilla y estilo validados contra el catálogo. Demo visual sin IA: `npx tsx scripts/demo-titulares.ts`.
  - **Borradores persistentes** (`/gemini/cover-drafts` CRUD): cada generación se auto-guarda sobre `community_content` (kind `portada_borrador`, mismo patrón que historias/carruseles — SIN migración de DB): imagen completa en `imageUrl`, thumbnail webp + settings del set en `data`. La lista viaja solo con thumbnails; poda automática a los últimos 60. El frontend auto-guarda al generar, actualiza el borrador al ajustar por chat, y tiene panel de cargar/eliminar.
  - **Motor v2 en Historias y Posts IA** (`routes/community/index.ts`): los TÍTULOS de historias 9:16 y slides de carruseles se renderizan con el mismo motor de tipografía de impacto (capa `construirOverlayTitular` compositada aparte; sub-copy/CTA/hashtags conservan su render Inter). `estilo_titular` opcional en generar/reintentar (un estilo por historia/carrusel completo, persistido en `data` para que los reintentos mantengan el diseño); selector compartido `EstiloTitularPicker` en las 3 páginas.
- **Historias — sistema narrativo v3** (julio 2026): rediseño de raíz por feedback del usuario ("falta coherencia entre una historia y otra", "todas dicen Hablamos por WhatsApp / hashtags / sigue viendo", "que sea imposible no quedarse viendo").
  - **Guion único** (`src/lib/story-script.ts`): antes el texto de cada frame se pedía al LLM POR SEPARADO Y EN PARALELO — ningún frame sabía qué decían los otros, de ahí la incoherencia. Ahora UNA sola llamada escribe el guion completo de la serie (hilo conductor, mismo protagonista, mismas cifras, progresión sin repetirse) y además dirige la ilustración de cada frame vía `prompt_visual`. `revisarGuion` detecta titulares repetidos/cifras faltantes y dispara una segunda pasada; hay fallback si el modelo no devuelve JSON usable.
  - **Formatos narrativos** (`src/lib/story-formats.ts`): 6 formatos con arcos definidos para 2-5 frames — caso real, antes y después, mito y verdad, autodiagnóstico, detrás de cámaras, momento de quiebre. Rotación FIFO afín al tipo de historia; cada paso del arco fija su layout.
  - **Layouts de historia**: 6 composiciones (clásico superior, editorial inferior, dato gigante, cita, respiro, cierre) que definen zona del titular, bloques a dibujar y las franjas que la ilustración debe dejar despejadas (el prompt de imagen las recibe: la escena se mueve según el layout). El botón de invitación y los hashtags SOLO aparecen en el frame de cierre — antes se pintaban los 4 bloques en todos los frames.
  - **Copy natural**: lista negra `FRASES_PROHIBIDAS` ("sigue viendo", "toca para seguir", "no te pierdas", "lleva tu negocio al siguiente nivel"…) aplicada en el prompt y limpiada en post-proceso; CTA variado que nace del relato, no siempre WhatsApp. Se eliminó el sistema viejo (`generarTextoHistoria`, `CTA_INTERMEDIO_INSTRUCTION`, `POSE_POR_ROL`).
  - **API/UI**: `estilo_titular` y `formato_narrativo` en `/community/historias/generar`; catálogo en `GET /community/historias/formatos`; el reintento recibe `guion_frame`, `hilo` y `otros_titulares` para no romper la serie. Demo visual sin IA: `npx tsx scripts/demo-historias.ts`.
  - **Texto secundario medido** (`ajustarTextoMedido` en title-style): sub-copy, CTA, hashtags y etiquetas de cifra se medían suponiendo "Inter", que NO está empaquetada — fontconfig caía en DejaVu Sans (más ancha) y el texto se salía del lienzo. Ahora se miden y dibujan con Montserrat Bold (empaquetada y calibrada, charset con minúsculas); el ajustador garantiza que ninguna línea supere el ancho disponible, partiendo palabras si hace falta.
  - **Modos de cierre rotativos** (`MODOS_CIERRE`): puerta abierta, auto-revisión, pregunta espejo, recurso, sin invitación y directo, con rotación FIFO y pesos; motivacional/comunidad rematan sin pedir nada la mayoría de las veces. WhatsApp queda vetado como cierre por defecto.
- **Historias — campaña anti-texto-cortado v4** (julio 2026): el usuario reportó "a veces salen portadas con los textos cortados o errores muy básicos". Se encontraron y corrigieron cinco causas distintas, todas con test de regresión.
  - **Recorte por palabra** (`recortarLimpio` en story-script): los límites del guion se aplicaban con `.slice(0, 44)`, que partía la última palabra a media letra ("…y el teléfono en si") — ESA era la causa directa de los textos cortados. Ahora se corta en el último espacio que quepa, se podan preposiciones colgantes y puntuación suelta, y una palabra larga se devuelve entera (el motor la achica). `sanearDato` exige cifra numérica ("cuarenta" ya no se dibuja como "cuarent") y `sanearHashtags` normaliza a 5 máximo. Todo frame pasa por `sanearFrameGuion`, venga del modelo, de un borrador o editado a mano.
  - **Métricas de fuente mal calibradas**: la sonda de calibración medía `tinta("XcX") - tinta("XX")`, que incluye el kerning de los pares X-c y c-X. En Montserrat (tabla de kerning rica) cada avance salía ~8px corto de 200 y el ancho de una línea se subestimaba **5-6%**. Ahora se mide con dos glifos iguales (`tinta("cc") - tinta("c")`), se toma el estimador mayor, se aplica 1.5% de margen y el script **valida contra frases de control y falla** si subestima. Además el charset se amplió con raya larga, puntos suspensivos, comillas tipográficas y demás signos que el modelo escribe (antes caían en el promedio de la fuente: la raya larga mide 1.01 y se estimaba 0.59).
  - **Sangrado de los efectos** (`sangradoEstilo`): el contorno, la sombra desplazada, la extrusión y las placas pintan más ancho que el avance de los glifos y no se descontaban del ancho de la zona; el font-size se redondeaba con `round`, ensanchando la línea hasta 6px de más. Ahora el ancho útil descuenta el sangrado, el redondeo es `floor` y las líneas alineadas a la izquierda arrancan desplazadas (la placa del estilo *placas* se salía 29px por el borde).
  - **Bloques inferiores apilados** (`apilarBloquesInferiores` en story-render): sub-copy, invitación y hashtags usaban centros fijos del layout con alturas variables; ahora se apilan de abajo hacia arriba con las alturas medidas, aire mínimo garantizado y margen contra el borde. El scrim inferior sube con la pila.
  - **Compositor testeable** (`src/lib/story-render.ts`): `componerHistoria`/`renderTextoEnHistoria` salieron de la ruta para poder rasterizar en tests sin Express ni IA. `story-render.test.ts` recorre 6 layouts x 7 estilos x 10 textos hostiles, rasteriza con librsvg y mide la caja de tinta contra la ZONA del layout; sus tolerancias se derivan de la geometría (inclinación, techo de altura, sangrado). Verificado por mutación: al desactivar cualquiera de las seis protecciones, el test falla. `scripts/demo-historias.ts` ya usa el compositor real en vez de una copia. Se borró el código muerto de estimación por ratio (`fitTextBlock`, `fitHashtagsBlock`, `renderTextBlockSvg`) y el `'Inter'` residual del contador.
  - **Otros arreglos**: bucle infinito en `partirPorAncho` si un solo carácter no cabía; clamp del bloque dentro del lienzo contando la inclinación; el layout de cifra gigante cede su espacio al titular si no hay cifra válida; la UI ya no descarta el guion nuevo al reintentar (reenviaba el viejo y revivía el texto anterior).

  - **Regla de honestidad**: sin un caso real verificado, el protagonista es un ESCENARIO ILUSTRATIVO — prohibido escribir "caso de éxito"/"testimonio"/"cliente real", atribuir cifras de resultado a la agencia o nombrar negocios reales.
...
- **Generador de Cotizaciones PDF** (`artifacts/api-server/src/routes/cotizaciones/`): Cotizaciones pixel-perfect replicando el diseño del PDF M&M Moda. Arquitectura: LLM (gpt-4.1) genera SOLO JSON de contenido (schema Zod, máx 2 reintentos); el servidor hace TODA la matemática (IVA 19% por módulo, split de pagos, formato CLP) en `finance.ts`; plantilla HTML/CSS fija (`template.ts`) + Puppeteer A4 (`pdf.ts`). Rutas (auth + requireArea ceo/ejecutivo): POST `/api/cotizaciones/generar` (aiLimiter) → `{cotizacion, intentos, html, htmlError}`; POST `/preview` → `{html}` (400 con detalles Zod); POST `/pdf` (uploadLimiter, Puppeteer) → PDF binario; GET `/ejemplo` (fixture M&M Moda). Netos pendientes = -1 bloquean preview/PDF hasta editar el JSON. **Cotización express (Task #162)**: si no se entregan precios, el LLM los ESTIMA con una GUÍA DE PRECIOS (rangos netos CLP por tipo de servicio en el SYSTEM_PROMPT) — nudge de reintento si deja netos -1 (el último intento acepta -1 como pendiente en vez de fallar); si no llega `esquema_pago` el server aplica default 50/50 (AL INICIAR / CONTRA ENTREGA); `validarPreciosEntregados` exige exactos los precios, mensualidad Y porcentajes del esquema entregados. Frontend: wizard "Nuevo contrato" en `/contratos` (`pages/hub/sheet-content.tsx`) con 2 pasos (Contexto → Vista previa): paso 1 pide solo cliente* + descripción* (o notas de reunión) con "Opciones avanzadas" colapsadas (nombre proyecto, fecha/asesor, módulos, % pago, mensualidad, vigencia, notas) y botón "✨ Generar cotización con IA" (envía `esquema_pago` solo si % ≠ 50); paso 2 muestra iframe con el HTML, hint 💡 cuando los precios fueron estimados, editor JSON → `/preview`, y botón final → `/pdf` → Drive + contrato (título con fallback: proyecto → `portada.alcance_titulo` → "Cotización {cliente}"). Tests en `generar.test.ts` (7, mock OpenAI + mock ./pdf). El antiguo generador jsPDF client-side fue eliminado (jspdf sigue en uso solo en videos.tsx).
- **Asistencia / Jornada del equipo**: Check-in/check-out de jornada con autodeclaración "estoy conectado en Discord", checklist diario "¿qué hiciste hoy?" e historial por integrante. Backend en `routes/jornada.ts` con self-service en `/api/jornada/*` (montado FUERA del gate de área de `/hub`, para que TODAS las áreas aprobadas puedan marcar) y supervisión (`/jornada/overview`, `/jornada/history` de terceros) gateada dentro del router a ceo/ejecutivo/rrhh/superadmin. Convenciones: día local America/Santiago (`work_date`/`log_date` YYYY-MM-DD, la sesión pertenece al día del check-in), semana lunes→domingo, sesiones abiertas con tope de 16 h (salidas olvidadas no inflan horas; UI avisa "stale"), índice único parcial = 1 sesión abierta por usuario (doble check-in → 409). Frontend: tarjeta "Mi jornada" en Mi Día + chip compacto en el sidebar del panel (`components/jornada-card.tsx`, query key `["jornada-me"]`), y la página `/asistencia` con pase de lista del día (estado/llegada/salida/horas/Discord/checklist), matriz semanal por integrante e historial 7/30/92 días. Tests en `jornada.test.ts`. **Verificación Discord opcional**: bot propio del cliente (secret `DISCORD_BOT_TOKEN`, opcional `DISCORD_GUILD_ID` si el bot está en varios servidores) verifica presencia en **canal de voz** vía REST (sin gateway) al marcar entrada y con un barrido cada 10 min que acumula `discord_checks/discord_hits` por sesión (`lib/discord.ts`, `lib/discord-sweep.ts`, arrancado en `index.ts`); emparejamiento usuario↔miembro del servidor lo hace un admin en la pestaña Asistencia (rutas `/jornada/discord/status|members|map`, requiere "Server Members Intent" para listar miembros); usuarios sin emparejar siguen con autodeclaración (checkbox). Nunca bloquea el check-in: si no se puede verificar, `discordVerified` queda null. Migraciones `0018_hub_attendance.sql` y `0019_discord_attendance.sql` (aplicadas en dev; pendientes en prod al publicar).
...
| Rol | Pantalla | Alrededor de qué está construida |
|---|---|---|
| CEO | `/` | El dashboard general — el sidebar da acceso además a todas las secciones del Hub (Proyectos, Clientes, Ventas, Cobros, Servicios, Contratos, Reuniones, Notas, Equipo, Asistencia, Drive, Torre CEO) |
...
| Ventas | `/dashboard-ejecutivo` | Landing del Hub — el sidebar suma Proyectos, Clientes, Contratos (wizard, PDF y chat IA), Cobros, Servicios, Reuniones, Notas, Equipo y Drive, recortado a su alcance |
| Programador | `/mis-tareas` | Scrumban por etapa y requerimientos técnicos |
| Contador | `/reportes` | Neto, IVA, facturación por mes, vencimientos y cobranza |
| RRHH | `/rrhh` | Fichas laborales, asistencia y solicitudes de acceso |

#### Cada pantalla ESCRIBE: nadie depende de otra sección para trabajar
Las pantallas de área nacieron como tableros de lectura y eso obligaba a saltar
al gestor de videos o al Hub para cualquier acción real. Ahora cada rol cierra
su ciclo completo desde su propia página:

| Rol | Qué puede hacer sin salir de su pantalla |
|---|---|
| Editora | Crear el video, completar los textos que faltan (título de YouTube, descripciones de TikTok/Instagram) y **enviar a revisión** eligiendo revisor, con mensaje. El archivo y la portada siguen en el gestor: necesitan subida, no un input |
| Marketing | **Aprobar o pedir cambios** ahí mismo, con el motivo viajando en la notificación a quien editó. Era el cuello de botella del contenido: si hay que abrir otra pantalla para aprobar, no se aprueba |
| Redes | **Programar, reprogramar y quitar fecha** desde las tres listas (atrasados, próximas, aprobados sin fecha) y **marcar en qué red salió** realmente cada publicación |
| Programador | Mover tareas de etapa, crear tareas (también **desde un módulo del brief**, con sus requisitos como notas), editar notas y criticidad, e informar avance de proyectos sin tareas. Más dos señales que el tablero no daba: **WIP** (cuántas cosas abiertas a la vez) y **estancadas** (+3 días sin moverse) |
| Contador | Registrar **cobranza** por contrato (por facturar / facturado / pagado / incobrable, N° de factura, fecha de pago y nota), con KPIs de "por cobrar" y "cobrado" y la columna en el CSV |

**Confidencialidad de la cobranza**: el contador NO tiene escritura sobre
`contracts` — tiene `PATCH /api/hub/contracts/:id/cobro`, un endpoint acotado
que solo escribe el sub-objeto `cobro` y exige `canSeeMoney`. Así lleva la
cobranza sin poder tocar precios, módulos ni alcance. El objeto `cobro` se
elimina en `redactContractMoney`: para quien no ve montos, las facturas y los
pagos tampoco existen.

**Escritura sobre el tablero compartido** (`lib/hub-write.ts`): `/mis-tareas`
pasó de `/api/hub/owner` a `/api/hub`, que además devuelve la versión y los
`writeScopes` del rol. Se manda la colección completa y el servidor fusiona
entidad por entidad con `baseVersion`; la regla al modificar es **siempre subir
`updatedAt`**, o la copia guardada gana la fusión.

**Corregido**: `router.use("/hub", requireArea(...))` gateaba también el tablero
en sí, así que marketing —que tiene `hubScopes` pero pertenece al área
`marketing`— recibía 403 en las tarjetas de proyectos y cartera de su propia
pantalla. `lib/hub-gate.ts` deja `/hub` y `/hub/owner` gateados por ROL (que ya
recorta colecciones y censura montos) y mantiene el gate por área en el resto
(IA, briefs, cobranza). Con test de regresión: ningún rol con `hubScopes` puede
quedar fuera del tablero por su área.

Invariantes con test: ningún rol comparte pantalla de inicio con otro, cada uno entra a la suya, todos ven `/mi-dia` y `/tickets`, y nadie entra a la pantalla de otra área.

**Cerrado en esta pasada**: `/api/credentials` no tenía ningún control de rol — cualquier sesión autenticada podía listar, sobrescribir o borrar las claves de API de todas las redes. Ahora exige dirección en el servidor, y `/ajustes` salió de `COMMON_ROUTES` (guarda credenciales y gestión de usuarios).
...
- **RRHH — fichas ampliadas, reportes diarios e informe semanal** (migración `0039_rrhh_reportes.sql`, aplicada en dev; pendiente en prod al publicar — el Publish sincroniza el esquema dev→prod por diff automático, NO se agrega DDL al arranque):
  - Ficha: campos opcionales `personal_email`, `company_email`, `rut`, `birth_date` (texto, validados en `routes/hr/index.ts`).
  - Reportes diarios (`hr_daily_reports`, router `routes/hr/reportes.ts`): `GET/POST /api/hr/reportes`, `PATCH /api/hr/reportes/:id`. Solo al CREAR se avisa: notificación a CEOs (excluye al autor) + correo a `CORREO_DIRECCION` (webmakerchile@gmail.com); el resultado del correo queda EN la fila (`email_status`: `""|enviado|fallido|sin_configurar` + `email_detail`) y jamás bloquea el guardado. Editar no re-avisa. HTML del correo escapado. Sin `recordActivity` a propósito: el contenido de RRHH no va a la bitácora del equipo.
  - Informe semanal (`hr_weekly_reports`): una fila por semana (`week_key` = lunes `YYYY-MM-DD`, único; semanas de America/Santiago), 3 secciones de texto, upsert `PUT /api/hr/informes/:week`, semanas pasadas navegables. Fechas validadas como fecha civil real (un `2026-02-30` normalizado caería en lunes y corrompería la llave).
  - Frontend: `components/rrhh-reportes.tsx` (el editor semanal se monta solo cuando la query resolvió y se remonta con `key={semana}` para no pisar datos del servidor).
- Tests: `src/lib/roles.test.ts` (invariantes de rutas/permisos), `src/routes/hub/owner.test.ts` (recorte por rol), `src/routes/hr/people.test.ts` (permisos, validación y reglas de RRHH) y `src/routes/hr/reportes.test.ts` (reportes/informes: gates, aviso solo al crear, correo que nunca bloquea, fechas).
...
- **Generación automática**: el wizard genera en una sola pasada la cotización comercial y el brief técnico, y sube **ambos** PDFs a Drive (`pages/hub/sheet-content.tsx`). Al regenerar los documentos tras el chat IA, el brief se rehace en la misma operación: las dos versiones nunca se desincronizan. Nadie tiene que acordarse de escribir requerimientos.
...
### Hub — secciones integradas al sidebar principal
Lo que fue un shell autocontenido en `/ejecutivo` ahora son páginas propias del panel, una por sección, cada una montada con el `Layout` y el sidebar principal como cualquier otra pantalla: `/dashboard-ejecutivo`, `/torre-ceo` (solo CEO), `/proyectos` (Kanban·Lista·Scrumban), `/clientes`, `/ventas`, `/cobros`, `/servicios`, `/contratos`, `/reuniones`, `/notas`, `/equipo-hoy`, `/asistencia`, `/drive-hub`. La visibilidad de cada una en el sidebar sigue las mismas rutas por rol de `lib/roles` — no hay una lista de permisos aparte de esa. El código compartido (estado, fetch, sheets, búsqueda global, toasts) vive en `admin-panel/src/pages/hub/` (`use-hub-board.ts`, `hub.css`, `sheet-content.tsx`, `global-search.tsx`, `views/*`); cada página solo fija qué pestaña muestra. Todo el estado sigue viviendo en un único blob JSON por usuario (`hub_state.data`, máx 2 MB) expuesto por `GET/PATCH /api/hub`, con espejo en localStorage (`wm_hub_v3`). El modo pantalla completa del Kanban (`/proyectos`) oculta el sidebar (`chromeHidden` en `Layout`) en vez de depender de un shell propio.
...
- `hub_work_sessions` - Sesiones de jornada del equipo (check-in/out, día local Santiago en `work_date`, flag `on_discord`; índice único parcial: máx. 1 sesión abierta por usuario; verificación Discord: `discord_checkin`, `discord_checks`, `discord_hits`, `discord_last_seen_at`. En `users`: `discord_user_id` único parcial + `discord_tag` para el emparejamiento)
- `hub_day_logs` - Checklist diario "qué hice hoy" por integrante (`log_date`, `text`, `done`)
