...
- **Tests** (Vitest): `pnpm --filter @workspace/api-server test` — 7 tests across 4 files. Covers AI rate-limit (429 + Retry-After on `/api/content/hashtag-suggestions`), publish rate-limit on `/api/youtube/publish`, brand-tone helper, scheduler json-parsing/error paths and a YouTube publish fetch-error path, all with mocked `fetch`. Config: `artifacts/api-server/vitest.config.ts`.
- **DB backups**: `scripts/backup-db.sh` runs `pg_dump` against `DATABASE_URL`, gzips to `BACKUP_DIR` (default `./backups`), prunes files older than `BACKUP_RETENTION_DAYS` (default 7), and — when `BACKUP_S3_URI` is set and the `aws` CLI is available — uploads the dump off-host via `aws s3 cp`. Restore: `gunzip -c backups/webmaker-YYYYMMDD-HHMMSS.sql.gz | psql "$DATABASE_URL"`. Schedule with cron, e.g. `0 3 * * * cd /path/to/project && ./scripts/backup-db.sh >> backups/backup.log 2>&1`.
