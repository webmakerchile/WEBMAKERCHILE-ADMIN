...
- **Tests** (Vitest, 18/18 verde): `pnpm --filter @workspace/api-server test`. Cubre handler-level real contra el router de library para `/api/library/templates/ai-fill` (400 sin redes, 400 sin name/base, 200 con campos saneados y drop de claves no esperadas, 200 recuperando JSON envuelto en texto, 502 sin filtrar detalle del error de OpenAI) y `/api/content/hashtag-suggestions` (400/200/500 con DB+Gemini mockeados); rate-limit AI sobre `/api/content/hashtag-suggestions`, `/api/analytics/insights`, `/api/gemini/generate-image` y `/api/gemini/conversations/:id/messages`; rate-limit publish sobre `/api/youtube/publish`; brand-tone helper; scheduler json-parsing/error paths con `fetch` mockeado. Config: `artifacts/api-server/vitest.config.ts`.
