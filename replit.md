...
- **Generador de Cotizaciones PDF** (`artifacts/api-server/src/routes/cotizaciones/`): Cotizaciones pixel-perfect replicando el diseño del PDF M&M Moda. Arquitectura: LLM (gpt-4.1) genera SOLO JSON de contenido (schema Zod, máx 2 reintentos); el servidor hace TODA la matemática (IVA 19% por módulo, split de pagos, formato CLP) en `finance.ts`; plantilla HTML/CSS fija (`template.ts`) + Puppeteer A4 (`pdf.ts`). Rutas (auth + requireArea ceo/ejecutivo): POST `/api/cotizaciones/generar` (aiLimiter) → `{cotizacion, intentos, html, htmlError}`; POST `/preview` → `{html}` (400 con detalles Zod); POST `/pdf` (uploadLimiter, Puppeteer) → PDF binario; GET `/ejemplo` (fixture M&M Moda). Netos pendientes = -1 bloquean preview/PDF hasta editar el JSON. Frontend: wizard "Nuevo contrato" en `ejecutivo.tsx` con 4 pasos (Datos → Módulos → Pago → Vista previa): paso 3 llama `/generar` armando contexto desde los campos + notas de reunión; paso 4 muestra iframe sandbox con el HTML del servidor, editor JSON colapsable → `/preview`, y botón final → `/pdf` → sube a Drive (fallback descarga local) + guarda contrato. El antiguo generador jsPDF client-side fue eliminado (jspdf sigue en uso solo en videos.tsx).
