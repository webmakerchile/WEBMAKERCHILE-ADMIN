...
- **Error monitoring (Sentry)**:
  - Backend: `artifacts/api-server/src/lib/sentry.ts` exposes `initSentry()` called from `index.ts` before app load. App-level Express error handler in `app.ts` captures 5xx with route/method tags and user id. No-op when `SENTRY_DSN` is unset (logs status).
  - Frontend: `artifacts/admin-panel/src/lib/sentry.ts` initialised in `main.tsx`, with `<Sentry.ErrorBoundary>` wrapping `<App />`. No-op when `VITE_SENTRY_DSN` is unset.
  - Required secrets to enable: `SENTRY_DSN` (backend) and `VITE_SENTRY_DSN` (frontend, build-time).
- **Tests** (Vitest): `pnpm --filter @workspace/api-server test` — 5 tests across 3 files. Covers rate-limit middleware (429 + Retry-After on `/api/content/hashtag-suggestions`), brand-tone helper, and scheduler json-parsing/error paths with mocked `fetch`. Config: `artifacts/api-server/vitest.config.ts`.
- **DB backups**: `scripts/backup-db.sh` runs `pg_dump` against `DATABASE_URL`, gzips to `BACKUP_DIR` (default `./backups`), and prunes files older than `BACKUP_RETENTION_DAYS` (default 7). Restore: `gunzip -c backups/webmaker-YYYYMMDD-HHMMSS.sql.gz | psql "$DATABASE_URL"`. Schedule with cron, e.g. `0 3 * * * cd /path/to/project && ./scripts/backup-db.sh >> backups/backup.log 2>&1`. For off-host durability, point `BACKUP_DIR` to a mounted Object Storage volume or extend the script to `aws s3 cp` after dump (follow-up #62).
- **TypeScript hardening**: cleared `as any` casts on TikTok/Instagram/scheduler `res.json()` by introducing typed shapes in `src/lib/tiktok-types.ts` and `src/lib/platform-types.ts`. Other api-server TS fixes: instagram temp-serve string|string[], library youtubeTitle Record, studio multer callback returns, auth test-login return paths, community sharp Buffer types and recurse `best` typing. Pre-existing TS6305 stale-dist warnings on lib refs remain (unrelated — libs need their own typecheck pass; tracked separately).
- **Follow-ups still open**: Provide `SENTRY_DSN`/`VITE_SENTRY_DSN` secrets to activate monitoring (#61); wire backup script to external storage and cron (#62); broaden test coverage to per-publisher integration paths (#63).
