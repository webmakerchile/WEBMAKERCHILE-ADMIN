...
import { NotesView } from "./hub/views/notes-view";
...
const [TITLE, SUBTITLE] = TAB_TITLES.notes;

export default function NotasPage() {
  const hub = useHubBoard("notes");
  const [filterCat, setFilterCat] = useState("");
  const [searchQ, setSearchQ] = useState("");
  const onNew = hub.newAction("notes", { kind: "new-note" });
      ...
      <div className="hub-root relative">
        ...
        <div className="main">
          <NotesView
            state={hub.state}
            onSave={hub.setState}
            onOpen={id => hub.openSheet({ kind: "note", id })}
