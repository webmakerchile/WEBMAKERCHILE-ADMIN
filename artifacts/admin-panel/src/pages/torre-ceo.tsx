...
const [TITLE, SUBTITLE] = TAB_TITLES.torre;

export default function TorreCeoPage() {
  const hub = useHubBoard("torre");
    ...
    <Layout>
      <div className="hub-root relative">
        ...
        <div className="main">
          {hub.isCeo ? (
            <div className="wrap">
              <TorrePanel showToast={hub.showToast} />
            </div>
          ) : (
            <div className="wrap">
              <EmptyState
                title="Solo para dirección"
                hint="La Torre CEO muestra el semáforo por área y las metas de la empresa — solo el rol CEO puede verla."
                icon={<Lock />}
              />
            </div>
          )}
