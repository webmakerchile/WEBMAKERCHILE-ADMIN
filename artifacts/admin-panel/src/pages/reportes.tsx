...
import {
  useHubOwner, contractTotal, splitIva, fmtCLP, fmtDate, daysUntil,
...
export default function ReportesPage() {
  ...
  const exportCsv = () => {
    const head = ["Titulo", "Cliente", "Estado", "Firma", "Vencimiento", "Neto", "IVA", "Total"];
    const rows = scoped.map(c => {
      const total = contractTotal(c);
      const { neto, iva } = splitIva(total);
      return [c.title, c.client, c.status, c.signedAt || "", c.expiresAt || "", neto, iva, total];
    });
        ...
        {!isLoading && !error && (
          <>
            ...
            <p className="text-xs text-muted-foreground">
              ...
              Esta vista es de solo lectura: los contratos se editan en el Hub Ejecutivo.
            </p>
          </>
        )}
      </div>
