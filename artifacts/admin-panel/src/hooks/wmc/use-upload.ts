...
// Minimal structural stand-in for Uppy's UppyFile type. @uppy/core isn't a
// dependency of this monorepo and this hook's Uppy-plugin helper
// (getUploadParameters) isn't actually wired to a real Uppy widget by the
// ported pages — it only reads name/size/type off the file object, so a
// local shape avoids adding a real runtime dependency for a type-only need.
...
export function useUpload(options: UseUploadOptions = {}) {
  ...
  const requestUploadUrl = useCallback(
    async (file: File): Promise<UploadResponse> => {
      const response = await fetch("/api/wmc/uploads/request-url", {
        method: "POST",
        headers: {
