...
**Actualización — el guard viejo ya no existe:** `AreaGuard`/`EjecutivoRoute` y el Hub Ejecutivo (`ejecutivo.tsx`) fueron eliminados; hoy TODAS las rutas usan `RouteShell`/`lib/roles` (ver `hub-shell-to-sidebar-migration.md`). La tensión "dos guards compitiendo" descrita arriba es historia, no el estado actual — no asumir que `AreaGuard` sigue interceptando nada sin confirmarlo con grep primero.
